...
## ⚙️ Configuración requerida de MRP

Para que el addon funcione correctamente, los siguientes parámetros de **MRP** deben estar activados en `Ajustes → Fabricación`:

### 🏭 Órdenes de trabajo — `work_orders_enabled`

| Parámetro | `mrp.work_orders_enabled` |
|---|---|
| **Ubicación** | Fabricación → Ajustes → Operaciones |
| **Descripción** | Habilita las **órdenes de trabajo (WO)** dentro de las órdenes de fabricación. Este addon crea automáticamente WO de corte de largo, corte de ancho y ensamblaje. Sin esta opción, las WO no se generan y el flujo de fabricación se rompe. |
| **Valor requerido** | ✅ Activado |

### 🔗 Dependencias entre órdenes de trabajo — `work_orders_dependencies`

| Parámetro | `mrp.work_orders_dependencies` |
|---|---|
| **Ubicación** | Fabricación → Ajustes → Operaciones |
| **Descripción** | Permite definir **dependencias entre WO** (una WO espera a que otra termine). El addon las usa para secuenciar: corte de largo → corte de ancho → ensamblaje. Sin esta opción, las WO se crean pero no se encadenan correctamente. |
| **Valor requerido** | ✅ Activado |

### 📦 Subproductos — `use_subcontracting` / `by_products`

| Parámetro | `mrp.by_products` |
|---|---|
| **Ubicación** | Fabricación → Ajustes → Operaciones |
| **Descripción** | Habilita la gestión de **subproductos** en las MO. El addon genera movimientos de sobrantes/recortes (`leftover`) como subproductos automáticamente. Sin esta opción, los recortes no se registran en la MO. |
| **Valor requerido** | ✅ Activado |

> **⚠️ Nota:** Si estos parámetros no están activados, el wizard de generación de MO puede fallar silenciosamente o producir órdenes incompletas. Verifica siempre que estén encendidos antes de usar el addon.

---
## 🧮 Optimización de cortes

El addon incluye un **optimizador de cortes** (`cut_optimizer.py`) que planifica automáticamente la disposición de cortes para minimizar el desperdicio de material.

### Algoritmo

1. **First-Fit Decreasing (FFD)** — Agrupa los cortes requeridos en la menor cantidad posible de piezas de stock
2. **Mejora iterativa** — Intercambia cortes entre piezas para reducir aún más el número de piezas usadas
3. **Encadenamiento de restos** — Los restos aprovechables (≥ 50 cm / 19.685") se reutilizan como stock para cortes más pequeños

### Flujo de creación de variantes

```
1. Analizar componentes del panel y sus largos requeridos
2. Optimizar agrupación de cortes (bin packing)
3. Por cada pieza de stock:
   ├── Crear variante de producto con el largo exacto de corte
   ├── Asignar como materia prima a la MO
   └── Si sobra >= 50 cm:
       ├── Crear variante de "resto" (wf_panel_leftover_stock)
       └── Registrar como subproducto en la MO
   └── Si sobra < 50 cm:
       └── Descartar como desperdicio
4. Archivar variantes de corte consumidas (sin stock)
```

### Ejemplo

| Stock disponible | Cortes requeridos | Plan optimizado | Resto |
|---|---|---|---|
| Perfil 240" | 96" + 48" + 48" + 24" | 1 pieza: 96"+48"+48"+24" = 216" | 24" (< 50cm → descartado) |
| Perfil 240" | 96" + 96" + 48" | 1 pieza: 96"+96" = 192" | 48" (≥ 50cm → resto aprovechable) |
| | | 2ª pieza: 48" | 192" (resto del paso anterior) |

---
## � Costo por hora en centros de trabajo

Para que el addon calcule correctamente el **costo por producto**, cada **centro de trabajo (work center)** debe tener configurado un **costo por hora**. Este valor se usa junto con la duración planificada de cada WO para determinar el costo de manufactura.

### Centros de trabajo requeridos

| Centro de trabajo | Costo por hora |
|---|---|
| Corte de largo | **$200.00** |
| Corte de ancho | **$200.00** |
| Ensamblaje | **$200.00** |

### Configuración

1. Ve a **Fabricación → Configuración → Centros de trabajo**
2. Abre cada centro de trabajo (Corte largo, Corte ancho, Ensamblaje)
3. En el campo **"Costo por hora"** (`costs_hour`), ingresa **200.00**
4. Guarda los cambios

> **💡 El costo por hora se multiplica por la duración planificada** (`wf_planned_duration_min`) de cada WO para calcular el costo planificado total de la MO. Sin este valor, el costo planificado aparecerá como $0.00.
